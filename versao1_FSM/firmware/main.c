...
// === VARIÁVEIS GLOBAIS PARA ARMAZENAR OS VETORES ===
...
// FUNÇÃO RELATÓRIO COMPLETO VERSÃO 1 
static void relatorio_completo(void) {
    printf("\n");
    printf("=========================================================\n");
    printf("|              RELATÓRIO COMPLETO - VERSÃO 1            |\n");
    printf("=========================================================\n");
    printf("|              ARQUITETURA: FSM SEQUENCIAL              |\n");
    printf("=========================================================\n");
    ...
    uint32_t latency_before = produto_escalar_ciclos_latencia_read();  
    uint32_t total_ops_before = produto_escalar_total_operacoes_read(); 
    ...
    uint32_t latency_after = produto_escalar_ciclos_latencia_read();    
    uint32_t total_ops_after = produto_escalar_total_operacoes_read();  
    uint32_t measurement_valid = produto_escalar_medicao_valida_read(); 
    
    // MEDIÇÕES DE PERFORMANCE
    printf("\n--- MEDIÇÕES DE PERFORMANCE ---\n");
    
    uint32_t clock_freq = 60; // MHz
    ...
    if (measurement_valid && latency_after > 0 && total_ops_after > 0) {
        // Throughput (MOPS) = (operações por lote / latência em ciclos) * frequência do clock (MHz)
        uint32_t throughput_mops = (total_ops_after * clock_freq) / latency_after;
        
        // Eficiência = throughput (MOPS) / frequência (MHz)
        // Resultado em MOPS/MHz, multiplicado por 100 para formar parte inteira e decimal
        uint32_t efficiency_x100 = (throughput_mops * 100) / clock_freq;
        
        // Tempo total (ns) = latência (ciclos) * período do clock (ns)
        // Período = 1000 / frequência(MHz)
        uint32_t time_total = (latency_after * 1000) / clock_freq;

        // Tempo por operação (ns) = (latência / operações) * período
        // Multiplicado por 10 para extrair parte fracionária
    ...
    // RESUMO PARA TCC - CÁLCULOS CORRIGIDOS
    printf("\n--- RESUMO PARA TCC ---\n");
    printf("Arquitetura: FSM Sequencial (Versão 1)\n");
    printf("Clock: %ld MHz\n", (long)clock_freq);
    printf("Latência medida: %ld ciclos\n", (long)latency_after);
    ...
    uint32_t throughput_mops = (total_ops_after * clock_freq) / latency_after;
    uint32_t efficiency_x100 = (throughput_mops * 100) / clock_freq;
    uint32_t ideal_throughput = clock_freq; // 1 op/ciclo
...
static void debug_csr(void) {
    ...
    // Verifica quais CSRs existem - ATUALIZADO
#ifdef CSR_PRODUTO_ESCALAR_CICLOS_LATENCIA_ADDR
    printf("CICLOS_LATENCIA_ADDR definido: 0x%08lx\n", (unsigned long)CSR_PRODUTO_ESCALAR_CICLOS_LATENCIA_ADDR);
#else
    printf("CICLOS_LATENCIA_ADDR NÃO definido\n");
#endif

#ifdef CSR_PRODUTO_ESCALAR_TOTAL_OPERACOES_ADDR
    printf("TOTAL_OPERACOES_ADDR definido: 0x%08lx\n", (unsigned long)CSR_PRODUTO_ESCALAR_TOTAL_OPERACOES_ADDR);
#else
    printf("TOTAL_OPERACOES_ADDR NÃO definido\n");
#endif

#ifdef CSR_PRODUTO_ESCALAR_MEDICAO_VALIDA_ADDR
    printf("MEDICAO_VALIDA_ADDR definido: 0x%08lx\n", (unsigned long)CSR_PRODUTO_ESCALAR_MEDICAO_VALIDA_ADDR);
#else
    printf("MEDICAO_VALIDA_ADDR NÃO definido\n");
#endif
    ...
    // Testa leitura das novas CSRs 
    printf("Leitura CSRs de medição:\n");
    printf("  ciclos_latencia: 0x%08lx\n", (unsigned long)produto_escalar_ciclos_latencia_read());
    printf("  total_operacoes: 0x%08lx\n", (unsigned long)produto_escalar_total_operacoes_read());
    printf("  medicao_valida: 0x%08lx\n", (unsigned long)produto_escalar_medicao_valida_read());
}
