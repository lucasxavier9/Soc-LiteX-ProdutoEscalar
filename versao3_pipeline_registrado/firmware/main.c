...
static void calc_produto_escalar(void)
{
    ...
    for(int i = 0; i < max_wait; i++) {
        ...
        // Fallback: se measurement_valid ficar ativo, também consideramos concluído
        if (produto_escalar_measurement_valid_read()) {
            measurement_valid_flag = 1;
            break;
        }
        if (i % 5 == 0) printf(".");
    }
    ...
    if (concluido_flag) {
        printf(" Concluído!\n");
    } else if (measurement_valid_flag) {
        printf(" Concluído (via measurement_valid)!\n");
    } else {
...
// FUNÇÃO RELATÓRIO COMPLETO VERSÃO 3 - MESMO MÉTODO DAS VERSÕES 1 E 2
static void relatorio_completo(void) {
    printf("\n");
    printf("=========================================================\n");
    printf("|              RELATÓRIO COMPLETO - VERSÃO 3            |\n");
    printf("=========================================================\n");
    printf("|              ARQUITETURA: PIPELINE REGISTRADO         |\n");
    printf("=========================================================\n");
    ...
    // 1. EXECUTA O PRODUTO ESCALAR COM OS VETORES GLOBAIS
...
// Executa operação
    ...
    // Aguarda processamento - VERSÃO OTIMIZADA
    printf("Processando");
    int concluido_flag = 0;
    int measurement_valid_flag = 0;

    // ESPERA MAIS INTELIGENTE - Verifica ambos os sinais
    for(int i = 0; i < 8; i++) {  // Reduzido para 8 iterações
        for(volatile int d = 0; d < 1000; d++); // Delay mínimo
        
        // Verifica tanto 'concluido' quanto 'measurement_valid'
        if (produto_escalar_concluido_read() || produto_escalar_measurement_valid_read()) {
            concluido_flag = 1;
            measurement_valid_flag = produto_escalar_measurement_valid_read();
            break;
        }
        printf(".");
    }
    ...
    // 2. MEDIÇÕES DE PERFORMANCE - MESMO MÉTODO SIMPLES DAS VERSÕES 1 E 2
    printf("\n--- MEDIÇÕES DE PERFORMANCE ---\n");
    
    uint32_t clock_freq = 60; // MHz - FIXO COMO NAS VERSÕES 1 E 2
    ...
    // SÓ CALCULA SE MEDIÇÃO FOR VÁLIDA - MESMO CÁLCULO SIMPLES
    ...
    // RESUMO PARA TCC 
    printf("\n--- RESUMO PARA TCC ---\n");
    printf("Arquitetura: Pipeline Registrado (Versão 3)\n");
    printf("Clock: %ld MHz\n", (long)clock_freq);
    printf("Latência medida: %ld ciclos\n", (long)latency_after);
